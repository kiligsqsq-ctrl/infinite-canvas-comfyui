...
`canvas-agent` 由本仓库独立维护，源码版从当前仓库运行，桌面版将由安装程序一并提供。

## Codex MCP

如果希望 Codex 终端能直接操作画布，需要先把 Canvas Agent 注册成 Codex MCP。

直接运行 `bun run dev` 只启动本地 Agent 服务，不会安装 MCP，也不会增加 Codex 工具上下文。只有安装 Codex app 插件，或手动执行 `codex mcp add` 后，`infinite-canvas` 工具才会进入 Codex 上下文；由于工具较多，不使用时建议移除。
...
使用时可以直接在 Codex 里说“打开 Infinite Canvas”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后打开本仓库部署的画布并自动新建、连接画布。

Canvas Agent 启动后，给 Codex 添加 MCP：

```bash
codex mcp add infinite-canvas -- node /path/to/infinite-canvas-comfyui/canvas-agent/dist/index.js mcp
```
...
command = "npx"
args = ["/path/to/infinite-canvas-comfyui/canvas-agent/dist/index.js", "mcp"]
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- node /path/to/infinite-canvas-comfyui/canvas-agent/dist/index.js mcp
```
