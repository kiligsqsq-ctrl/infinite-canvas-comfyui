...
export function checkVersions() {
    ...
    if (!localCodexVersion) {
        ...
        logger.warn(`Bundled Codex ${CODEX_VERSION} does not match local Codex ${localCodexVersion}. Update Codex or rebuild Canvas Agent from this repository.`);
    }
...
async function checkLatestVersions(localCodexVersion: string) {
    try {
        ...
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Update this repository before rebuilding Canvas Agent.`);
