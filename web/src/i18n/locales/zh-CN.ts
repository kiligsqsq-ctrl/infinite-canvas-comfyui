export default {
    ...
    canvas: {
        ...
        createMenu: {
            fromNode: "引用该节点生成", close: "关闭", text: "文本生成", textDescription: "脚本、广告词、品牌文案", image: "图片生成", video: "视频生成", audio: "音频生成", config: "配置节点", configDescription: "模型、尺寸、数量和输入顺序", select: "选择节点", uploadAssets: "上传素材", uploadImage: "上传图片", uploadVideo: "上传视频", uploadAudio: "上传音频", uploadOnly: "只导入素材，不触发生成",
        },
        node: {
            node: "节点",
        ...
        configNode: { title: "生成配置", image: "生图", text: "文本", video: "视频", audio: "音频", prompt: "提示词", references: "参考图", videoReferences: "参考视频", audioReferences: "参考音频", items: "{{count}} 个", images: "{{count}} 张", compose: "组装提示词", preview: "检查", stop: "停止", generate: "开始生成" },
        taskCenter: {
            title: "生成任务", clearFinished: "清除已结束", empty: "暂无生成任务", untitled: "生成任务", focus: "定位节点", cancel: "取消任务",
            status: { queued: "等待中", running: "运行中", completed: "已完成", failed: "失败", canceled: "已取消" },
        },
        missingMedia: {
            title: "素材丢失", description: "这些素材文件不在当前浏览器的本地存储中。请选择原文件重新关联，节点和连线都会保留。", relink: "重新关联", none: "没有缺失素材", typeMismatch: "请选择{{kind}}文件", relinked: "素材已重新关联", relinkFailed: "重新关联失败",
        },
        generationPreview: {
            title: "生成前检查", start: "确认并开始生成", blocked: "发现需要先处理的问题", ready: "检查完成，可以提交生成",
            model: "模型", channel: "渠道", mode: "生成类型", output: "结果位置", workflow: "ComfyUI 工作流", outputNode: "输出节点 {{node}}",
            prompt: "最终提示词", emptyPrompt: "本次没有提示词", references: "本次实际参考素材（按发送顺序）", noReferences: "本次不发送参考素材", parameters: "生成参数",
            imageLimit: "图片 {{used}} / {{limit}}", videoLimit: "视频 {{used}} / {{limit}}", audioLimit: "音频 {{used}} / {{limit}}",
            failed: "无法读取本次生成信息", workflowMissing: "已绑定的 ComfyUI 工作流在本地找不到，请重新导入并保存映射", promptMissing: "本次任务没有可用提示词",
            referenceOverflow: "{{kind}}参考超过工作流限制：{{used}} / {{limit}}", imageKind: "图片", videoKind: "视频", audioKind: "音频", textReference: "文本 {{index}}",
            yes: "是", no: "否", writeBack: "结果会写回当前空节点", createBeside: "结果会在当前节点右侧创建新的{{kind}}节点",
            parameter: { count: "数量", size: "尺寸 / 比例", quality: "质量 / 清晰度", duration: "时长", generateAudio: "生成声音", voice: "声音", format: "格式", speed: "语速", reasoning: "推理强度" },
        },
        projectPage: {
        ...
        promptPanel: { video: "描述要生成的视频内容", audio: "描述要生成的音频内容", image: "描述要生成的图片内容", text: "请输入你想要生成的文本内容", editImage: "请输入你想要把这张图修改成什么", editText: "请输入你想要将本段文本修改成什么", expandEditor: "放大编辑", preview: "生成前检查", editorTitle: "编辑提示词", stopGeneration: "停止生成", generate: "生成", stop: "停止" },
    ...
    topNav: {
        ...
        updateErrors: {
            ...
            diverged: "本地分支和 kiligsqsq-ctrl/infinite-canvas-comfyui 的 main 分支已有不同修改，为避免覆盖，已停止自动更新。",
            notRepository: "当前安装目录不是 Git 仓库，无法使用一键更新。",
            unavailable: "本地更新服务不可用。请使用 bun run dev 启动画布后再试。",
            failed: "更新失败，请检查网络和 Git 配置。",
        },
    },
    home: {
        promptError: "获取提示词失败",
    ...
    config: {
        ...
        channels: {
            description: "本地服务和云端 API 可以同时使用；每个模型都能指定图片、视频、文本或音频能力。",
            add: "新增 API 渠道",
            localQuickTitle: "本地服务快捷接入",
            localQuickDescription: "选择你电脑上已经运行的服务，会自动填好常用地址；地址和模型仍可继续修改。",
            addPreset: "添加并配置",
            localTag: "本地",
            apiTag: "API",
            unnamed: "未命名渠道",
        ...
        serviceStatus: {
            title: "本地服务状态", description: "检查这台电脑上的 ComfyUI、Ollama 等本地服务是否可访问。", refresh: "重新检查", checking: "检查中", online: "在线 · {{latency}}ms", offline: "离线", models: "{{count}} 个模型", unknownError: "未知错误", empty: "尚未添加本地服务",
        },
        preferences: {
            interface: "界面偏好",
            language: "界面语言",
        ...
        channelEditor: {
            ...
            protocol: "协议",
            baseUrl: "接口地址",
            localKeyOptional: "（本地服务可留空）",
            localKeyPlaceholder: "本地服务通常不需要填写",
            localNoticeTitle: "浏览器会直接连接这台电脑上的服务",
            localNoticeDescription: "请先启动本地服务，并允许来自 http://localhost:3000 的跨域请求（CORS）。如果画布使用了其他端口，也要允许那个地址。",
            comfyCorsDescription: "请先启动 ComfyUI。若浏览器提示跨域错误，请让 ComfyUI 启动参数包含 --enable-cors-header http://localhost:3000；画布使用其他端口时请替换成实际地址。",
            testConnection: "测试连接",
            connectionAvailable: "连接成功，服务可以访问",
            connectionAvailableWithModels: "连接成功，发现 {{count}} 个模型",
            connectionFailed: "连接失败，请检查服务地址、启动状态和 CORS 设置",
            models: "渠道模型",
            modelDescription: "已选 {{count}} 个；请为每个模型指定能力。",
            comfyModelDescription: "已选 {{count}} 个；点击模型右侧的「上传 API 工作流」，即可使用你自己从 ComfyUI 导出的工作流。",
            selectModels: "选择模型",
            workflowReady: "API 工作流已连接",
            workflow: "上传 API 工作流",
        ...
        workflowMapper: {
            title: "{{model}} · ComfyUI 工作流映射", untitled: "未命名工作流",
            noticeTitle: "请导入 ComfyUI 的 API 格式工作流 JSON", noticeDescription: "工作流只保存在当前浏览器本地，不会上传到 GitHub 或其他服务器。保存后会优先使用你上传的工作流；生成时按画布连线顺序填入素材，并删除本次没有使用的素材加载节点。",
            importFile: "导入工作流 JSON", replaceFile: "更换工作流 JSON", fileHint: "在 ComfyUI 中使用“保存（API 格式）”导出的 JSON。普通界面工作流 JSON 不能直接调用。", importedFile: "已导入文件", fileSummary: "{{file}} · {{count}} 个节点",
            imported: "已识别 {{count}} 个工作流节点", importFailed: "工作流文件读取失败", empty: "请先导入 API 格式工作流 JSON", name: "工作流名称",
            prompt: "提示词输入", duration: "时长输入", ratio: "比例输入", resolution: "分辨率 / 兆像素输入", seed: "随机种子输入", imageSlots: "图片参考槽位", videoSlots: "视频参考槽位", audioSlots: "音频参考槽位", noSlots: "没有槽位，可手动添加",
            outputNode: "结果输出节点", selectNode: "选择节点", clear: "解除当前映射", importFirst: "请先导入工作流 JSON", promptRequired: "请选择提示词输入位置", outputRequired: "请选择结果输出节点", missingRequired: "请先补齐这些必要位置：{{fields}}", saved: "工作流映射已保存", saveFailed: "工作流映射保存失败",
            miniMaxDetected: "已识别 MiniMax-H3 API 工作流", mappingDetected: "已完成工作流自动识别", mappedSummary: "已找到：{{fields}}", slotSummary: "参考槽位：{{images}} 图、{{videos}} 视频、{{audios}} 音频", missingSummary: "仍需手动指定：{{fields}}", separator: "、", none: "暂无",
            checkCompatibility: "检查当前 ComfyUI", nodesReady: "兼容：所需 {{count}} 种节点均已安装", nodesMissing: "当前 ComfyUI 缺少 {{count}} 种节点", compatibilityFailed: "无法检查 ComfyUI 节点", compatibilityConnectionHint: "请确认接口地址、ComfyUI 启动状态和 CORS 设置", modelFilesNotChecked: "这里只检查节点类型；模型、VAE、CLIP、LoRA 等文件仍需在 ComfyUI 中自行确认。",
        },
        modelSelect: {
            missingConfig: "请先填写接口地址；云端 API 还需要填写 API Key",
            ...
            comfyTitle: "ComfyUI 使用工作流，不提供标准模型列表",
            comfyDescription: "预设已加入 MiniMax-H3 模型。请选择模型右侧的「上传 API 工作流」，导入你自己的 ComfyUI 工作流。",
        ...
        localPresets: {
            comfyui: { name: "ComfyUI · MiniMax-H3", description: "适用于官方版、源码版、桌面版及第三方整合包；默认使用 ComfyUI 常见的 8188 端口。" },
            ollama: { name: "Ollama", description: "连接本机 Ollama 的 OpenAI 兼容接口，可直接拉取已经安装的模型。" },
            lmStudio: { name: "LM Studio", description: "连接 LM Studio 本地服务器，可直接使用已加载的 OpenAI 兼容模型。" },
            localOpenAI: { name: "通用本地接口", description: "用于 vLLM、LocalAI 或其他兼容 OpenAI 接口的本地服务。" },
        },
        webdav: {
            title: "WebDAV 同步",
