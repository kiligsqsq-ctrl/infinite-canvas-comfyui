import { Alert, App, Button, Checkbox, Input, Modal, Tabs } from "antd";
import { RefreshCw, Search } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { useTranslation } from "react-i18next";

import { fetchChannelModels } from "@/services/api/image";
import { channelRequiresApiKey, type ModelChannel } from "@/stores/use-config-store";
...
export function ModelSelectModal({ open, channel, selectedNames, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedNames: string[]; onConfirm: (names: string[]) => void; onClose: () => void }) {
    ...
    const isComfyUi = channel?.serviceType === "comfyui";
    ...
    const fetchModels = async () => {
        if (!channel) return;
        if (!channel.baseUrl.trim() || (channelRequiresApiKey(channel) && !channel.apiKey.trim())) {
    ...
    return (
        ...
        >
            <div className="flex flex-wrap items-center gap-3">
                ...
                <Button icon={<RefreshCw className="size-4" />} loading={loading} disabled={isComfyUi} onClick={() => void fetchModels()}>
                    {t("config.modelSelect.fetch")}
                </Button>
            </div>
            {isComfyUi ? <Alert className="mt-3" type="info" showIcon message={t("config.modelSelect.comfyTitle")} description={t("config.modelSelect.comfyDescription")} /> : <div className="mt-2 text-xs text-stone-500">{t("config.modelSelect.description")}</div>}
