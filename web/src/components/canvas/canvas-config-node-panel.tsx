...
import { ClipboardCheck, Image as ImageIcon, LoaderCircle, MessageSquare, Music2, Play, Settings2, Square, Video } from "lucide-react";
...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onPreview, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
            ...
            <div className="mt-auto grid grid-cols-[auto_minmax(0,1fr)] gap-2" onMouseDown={(event) => event.stopPropagation()}>
                <Button className="!h-9 !rounded-lg" icon={<ClipboardCheck className="size-4" />} disabled={isRunning || !canGenerate} onClick={() => onPreview(node.id)}>
                    {t("canvas.configNode.preview")}
                </Button>
                <Button type="primary" className="!h-9 !w-full !cursor-pointer !rounded-lg" danger={isRunning} disabled={!isRunning && !canGenerate} onClick={() => (isRunning ? onStop(node.id) : onGenerate(node.id))}>
