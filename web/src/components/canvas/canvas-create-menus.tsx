...
export function NodeCreateMenu({ position, onCreate, onUpload, onClose }: { position: Position; onCreate: (type: string) => void; onUpload: (kind: CanvasUploadKind) => void; onClose: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const { t } = useTranslation();
    ...
    return (
        ...
        >
            ...
            <div className="mx-1 my-2 border-t" style={{ borderColor: theme.node.stroke }} />
            <div className="px-1 pb-1 text-xs font-medium" style={{ color: theme.node.muted }}>
                {t("canvas.createMenu.uploadAssets")}
            </div>
            <div className="grid gap-1">
                <ConnectionCreateOption theme={theme} icon={<ImageIcon className="size-5" />} title={t("canvas.createMenu.uploadImage")} description={t("canvas.createMenu.uploadOnly")} onClick={() => onUpload("image")} />
                <ConnectionCreateOption theme={theme} icon={<Video className="size-5" />} title={t("canvas.createMenu.uploadVideo")} description={t("canvas.createMenu.uploadOnly")} onClick={() => onUpload("video")} />
                <ConnectionCreateOption theme={theme} icon={<Music2 className="size-5" />} title={t("canvas.createMenu.uploadAudio")} description={t("canvas.createMenu.uploadOnly")} onClick={() => onUpload("audio")} />
            </div>
        </div>
    );
}
