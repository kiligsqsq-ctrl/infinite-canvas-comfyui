...
import { ArrowUp, ClipboardCheck, LoaderCircle, Maximize2, Square } from "lucide-react";
...
type CanvasNodePromptPanelProps = {
    ...
    onPreview: (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => void;
...
export function CanvasNodePromptPanel({ node, isRunning, onPromptChange, onConfigChange, onGenerate, onPreview, onStop, mentionReferences = [], onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
        ...
        >
            ...
            <div className="mt-2 flex min-w-0 items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    <Tooltip title={t("canvas.promptPanel.preview")}>
                        <Button type="text" className="!h-8 !w-8 !min-w-8 shrink-0 !rounded-full !bg-transparent !p-0" style={{ color: theme.node.text }} icon={<ClipboardCheck className="size-3.5" />} disabled={isRunning || !prompt.trim()} onClick={() => onPreview(node.id, mode, prompt.trim())} aria-label={t("canvas.promptPanel.preview")} />
                    </Tooltip>
                    {mode === "image" ? (
                        <>
