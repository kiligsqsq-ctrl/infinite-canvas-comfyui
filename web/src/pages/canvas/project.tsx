...
import { CanvasGenerationPreviewModal } from "@/components/canvas/canvas-generation-preview-modal";
import { CanvasMissingMediaModal, findMissingCanvasMedia, type MissingCanvasMedia } from "@/components/canvas/canvas-missing-media-modal";
...
import { buildNodeMentionReferences, buildReferenceConnectionLabels, type CanvasResourceReference } from "@/lib/canvas/canvas-resource-references";
import { exportCanvasProjects } from "@/lib/canvas/canvas-export";
import { buildCanvasGenerationPreview, type CanvasGenerationPreviewData } from "@/lib/canvas/canvas-generation-preview";
...
import { ConnectionCreateMenu, NodeCreateMenu, type CanvasUploadKind, type PendingConnectionCreate } from "@/components/canvas/canvas-create-menus";
import {
    CanvasNodeType,
...
type CanvasGenerationTerminalStatus = "completed" | "failed";

type CanvasGenerationPreviewRequest = {
    nodeId: string;
...
function generationModeForNode(node?: CanvasNodeData): CanvasNodeGenerationMode {
...
function InfiniteCanvasPage() {
    ...
    const relinkTargetRef = useRef<MissingCanvasMedia | null>(null);
    const uploadTargetRef = useRef<{ position?: Position; kind?: CanvasUploadKind } | null>(null);
    ...
    const beginGenerationTask = useGenerationTaskStore((state) => state.beginTask);
    const setGenerationTaskStatus = useGenerationTaskStore((state) => state.setTaskStatus);
    ...
    const [missingMediaOpen, setMissingMediaOpen] = useState(false);
    ...
    const [generationPreview, setGenerationPreview] = useState<CanvasGenerationPreviewData | null>(null);
    const [generationPreviewRequest, setGenerationPreviewRequest] = useState<CanvasGenerationPreviewRequest | null>(null);
    ...
    const generationTaskIdsRef = useRef(new Map<AbortController, string>());
    ...
    const startGenerationRequest = useCallback((targetNodeId: string, originNodeId: string, runningId = originNodeId, controller = new AbortController()) => {
        ...
        if (previous?.controller !== controller) {
            ...
            if (previous && !Array.from(generationRequestsRef.current.values()).some((request) => request.targetNodeId !== targetNodeId && request.controller === previous.controller)) {
                const previousTaskId = generationTaskIdsRef.current.get(previous.controller);
                if (previousTaskId) setGenerationTaskStatus(previousTaskId, "canceled");
                generationTaskIdsRef.current.delete(previous.controller);
            }
        }
        generationRequestsRef.current.set(targetNodeId, { targetNodeId, originNodeId, runningNodeId: runningId, controller });
        if (!generationTaskIdsRef.current.has(controller)) {
            const originNode = nodesRef.current.find((node) => node.id === originNodeId);
            const taskId = beginGenerationTask({ projectId, runningNodeId: runningId, originNodeId, title: originNode?.title || t("canvas.taskCenter.untitled"), mode: generationModeForNode(originNode) });
            generationTaskIdsRef.current.set(controller, taskId);
            queueMicrotask(() => {
                if (!controller.signal.aborted) setGenerationTaskStatus(taskId, "running");
            });
        }
        return controller;
    }, [beginGenerationTask, projectId, setGenerationTaskStatus, t]);

    const finishGenerationRequest = useCallback((targetNodeId: string, controller: AbortController, terminalStatus: CanvasGenerationTerminalStatus = "completed") => {
        const request = generationRequestsRef.current.get(targetNodeId);
        if (request?.controller === controller) generationRequestsRef.current.delete(targetNodeId);
        if (Array.from(generationRequestsRef.current.values()).some((item) => item.controller === controller)) return;
        const taskId = generationTaskIdsRef.current.get(controller);
        if (!taskId) return;
        setGenerationTaskStatus(taskId, controller.signal.aborted ? "canceled" : terminalStatus);
        generationTaskIdsRef.current.delete(controller);
    }, [setGenerationTaskStatus]);

    const finishGenerationController = useCallback((controller: AbortController, terminalStatus: CanvasGenerationTerminalStatus = "completed") => {
    ...
    const stopGenerationByRunningId = useCallback((runningId: string) => {
        ...
        const affectedControllers = new Set<AbortController>();
        generationRequestsRef.current.forEach((request) => {
            if (request.runningNodeId !== runningId) return;
            request.controller.abort();
            affectedControllers.add(request.controller);
        ...
        affectedControllers.forEach((controller) => {
            const taskId = generationTaskIdsRef.current.get(controller);
            if (taskId) setGenerationTaskStatus(taskId, "canceled");
            generationTaskIdsRef.current.delete(controller);
        });
    ...
    const referenceLabelsByConnectionId = useMemo(() => {
        const labels = new Map<string, string>();
        const targetNodeIds = new Set(connections.map((connection) => connection.toNodeId));
        targetNodeIds.forEach((nodeId) => buildReferenceConnectionLabels(nodeId, nodes, connections).forEach((label, connectionId) => labels.set(connectionId, label)));
        return labels;
    }, [connections, nodes]);
    const missingMedia = useMemo(() => findMissingCanvasMedia(nodes), [nodes]);
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (target?.closest("input,textarea,select,[contenteditable='true'],[data-canvas-shortcuts-ignore],.ant-modal,.ant-drawer,.ant-popover,.ant-dropdown,.ant-select-dropdown,.ant-picker-dropdown")) return;
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            let terminalStatus: CanvasGenerationTerminalStatus = "completed";
            try {
                ...
                finishGenerationRequest(childId, controller, terminalStatus);
                setRunningNodeId(null);
            }
        },
    ...
    const handleUploadRequest = useCallback((position?: Position, kind?: CanvasUploadKind) => {
        uploadTargetRef.current = { position, kind };
        const input = imageInputRef.current;
        if (!input) return;
        input.accept = kind === "image" ? "image/*" : kind === "video" ? "video/*" : kind === "audio" ? "audio/*,.mp3,.wav" : "image/*,video/*,audio/*,.mp3,.wav";
        input.click();
    }, []);

    const requestMissingMediaRelink = useCallback((item: MissingCanvasMedia) => {
        relinkTargetRef.current = item;
        const input = relinkInputRef.current;
        if (!input) return;
        input.accept = item.kind === "image" ? "image/*" : item.kind === "video" ? "video/*" : "audio/*,.mp3,.wav";
        input.click();
    }, []);

    const handleRelinkInputChange = useCallback(async (event: ReactChangeEvent<HTMLInputElement>) => {
        const target = relinkTargetRef.current;
        const file = event.target.files?.[0];
        if (!target || !file) return;
        const matches = target.kind === "image" ? file.type.startsWith("image/") : target.kind === "video" ? file.type.startsWith("video/") : isAudioFile(file);
        if (!matches) {
            message.error(t("canvas.missingMedia.typeMismatch", { kind: t(`canvas.nodeTypes.${target.kind}`) }));
            event.target.value = "";
            return;
        }
        try {
            if (target.kind === "image") {
                ...
                setNodes((current) => current.map((node) => {
                    ...
                    const images = node.metadata?.images?.map((item) => item.id === target.imageId ? { ...item, status: NODE_STATUS_SUCCESS, content: image.url, storageKey: image.storageKey, naturalWidth: image.width, naturalHeight: image.height, bytes: image.bytes, mimeType: image.mimeType, missingMedia: false } : item);
                    const primaryImageId = node.metadata?.primaryImageId || node.metadata?.images?.[0]?.id;
                    const replacesPrimary = !target.imageId || target.imageId === primaryImageId;
                    return { ...node, title: replacesPrimary ? file.name || node.title : node.title, metadata: { ...node.metadata, ...(replacesPrimary ? imageMetadata(image) : {}), images, missingMedia: replacesPrimary ? false : node.metadata?.missingMedia } };
                }));
            } else {
                const media = await uploadMediaFile(file, target.kind);
                setNodes((current) => current.map((node) => node.id === target.nodeId ? { ...node, title: file.name || node.title, metadata: { ...node.metadata, ...(target.kind === "video" ? videoMetadata(media) : audioMetadata(media)), missingMedia: false } } : node));
            }
            message.success(t("canvas.missingMedia.relinked"));
            if (missingMedia.length <= 1) setMissingMediaOpen(false);
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("canvas.missingMedia.relinkFailed"));
        } finally {
            relinkTargetRef.current = null;
            event.target.value = "";
        }
    }, [message, missingMedia.length, t]);

    const handleImageInputChange = useCallback(
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            const target = uploadTargetRef.current;
            const files = Array.from(event.target.files || []).filter((file) => {
                if (target?.kind === "image") return file.type.startsWith("image/");
                if (target?.kind === "video") return file.type.startsWith("video/");
                if (target?.kind === "audio") return isAudioFile(file);
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const file = files[i];
                if (isAudioFile(file)) {
                    void createAudioFileNode(file, offsetPos);
                } else if (file.type.startsWith("video/")) {
                    void createVideoFileNode(file, offsetPos);
                } else {
                    void createImageFileNode(file, offsetPos);
                }
            }
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    finishGenerationRequest(nodeId, controller, terminalStatus);
                }
                return;
            }
            ...
            try {
                ...
                finishGenerationController(runController, "failed");
                setRunningNodeId(null);
                return;
            }
            const effectivePrompt = generationContext.prompt.trim();
            if (runController.signal.aborted) {
                finishGenerationController(runController);
                setRunningNodeId(null);
                return;
            }
            ...
            try {
                if (mode === "image") {
                    ...
                    if (hasFailure && !hasSuccess) terminalStatus = "failed";
                    if (hasFailure) {
                ...
                const answers = await Promise.all(
                    textTargetIds.map((targetNodeId) => {
                        ...
                        return requestImageQuestion(
                            ...
                            .finally(() => {
                                if (targetNodeId !== nodeId) finishGenerationRequest(targetNodeId, controller);
                            });
                    }),
                );
                ...
                finishGenerationController(runController, terminalStatus);
                setRunningNodeId(null);
            }
        },
        [effectiveConfig, finishGenerationController, finishGenerationRequest, isAiConfigReady, message, openConfigDialog, startGenerationRequest, t],
    );
    useEffect(() => {
    ...
    const handlePreviewGeneration = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                const request = { nodeId, mode, prompt };
                const preview = await buildCanvasGenerationPreview({ ...request, nodes: nodesRef.current, connections: connectionsRef.current, config: effectiveConfig });
                setGenerationPreviewRequest(request);
                setGenerationPreview(preview);
            } catch (error) {
                message.error(error instanceof Error ? error.message : t("canvas.generationPreview.failed"));
            }
        },
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            try {
                ...
                finishGenerationRequest(node.id, controller, terminalStatus);
                setRunningNodeId(null);
            }
        },
        [effectiveConfig, finishGenerationRequest, isAiConfigReady, message, openConfigDialog, startGenerationRequest, t],
    ...
    const renderNodePanel = useCallback(
        ...
        [configInputsById, confirmStopGeneration, handleConfigNodeChange, handleGenerateNode, handleNodePromptChange, handlePreviewGeneration, mentionReferencesByNodeId, renderPluginPanel, runningNodeId],
    );
    ...
    const renderNodeContentPanel = useCallback(
        (contentNode: CanvasNodeData) => (
            <CanvasConfigNodePanel
                ...
                onPreview={(nodeId) => {
                    ...
                    void handlePreviewGeneration(nodeId, target?.metadata?.generationMode || "image", target?.metadata?.composerContent ?? target?.metadata?.prompt ?? "");
                }}
                onGenerate={(nodeId) => {
                    const target = nodesRef.current.find((item) => item.id === nodeId);
        ...
        [configInputsById, confirmStopGeneration, handleConfigNodeChange, handleGenerateNode, handlePreviewGeneration, runningNodeId],
    );
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onRedo={redoCanvas}
                    projectId={projectId}
                    onCancelTask={stopGenerationByRunningId}
                    onFocusTask={focusNode}
                    missingMediaCount={missingMedia.length}
                    onOpenMissingMedia={() => setMissingMediaOpen(true)}
                ...
                >
                            ...
                            .map((connection) => {
                                ...
                                return (
                                    <ConnectionPath
                                        ...
                                        label={referenceLabelsByConnectionId.get(connection.id)}
                                        onSelect={() => {
                    ...
                    {nodeCreatePosition ? (
                        <NodeCreateMenu
                            ...
                            onUpload={(kind) => {
                                handleUploadRequest(nodeCreatePosition || undefined, kind);
                                setNodeCreatePosition(null);
                            }}
                ...
                <input ref={relinkInputRef} type="file" className="hidden" onChange={handleRelinkInputChange} />

                <CanvasMissingMediaModal open={missingMediaOpen} items={missingMedia} onRelink={requestMissingMediaRelink} onClose={() => setMissingMediaOpen(false)} />

                <CanvasNodeInfoModal node={infoNode} open={Boolean(infoNode)} onClose={() => setInfoNodeId(null)} />
                <CanvasPluginManagerModal open={pluginManagerOpen} onClose={() => setPluginManagerOpen(false)} />
                <CanvasGenerationPreviewModal
                    data={generationPreview}
                    onClose={() => {
                        setGenerationPreview(null);
                        setGenerationPreviewRequest(null);
                    }}
                    onConfirm={() => {
                        const request = generationPreviewRequest;
                        setGenerationPreview(null);
                        setGenerationPreviewRequest(null);
                        if (request) void handleGenerateNode(request.nodeId, request.mode, request.prompt);
                    }}
                />
