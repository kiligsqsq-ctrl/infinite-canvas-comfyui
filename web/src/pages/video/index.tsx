...
import { isMiniMaxH3VideoConfig, VideoSettingsPanel, normalizeMiniMaxResolutionValue, normalizeVideoResolutionValue, normalizeVideoSizeValue, videoResolutionLabel, videoSecondsLabel, videoSizeLabel } from "@/components/video-settings-panel";
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {isMiniMaxH3VideoConfig({ model, videoModel: model }) ? `${videoResolutionLabel(effectiveConfig.vquality, { model, videoModel: model })} · ` : `${normalizeResolution(effectiveConfig.vquality)}p · `}{videoSizeLabel(effectiveConfig.size)} · {videoSecondsLabel(effectiveConfig.videoSeconds)}
                                </span>
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    ...
    const miniMaxH3 = isMiniMaxH3VideoConfig({ ...config, model, videoModel: model });
    return {
        ...config,
        model,
        videoModel: model,
        size: seedance ? normalizeSeedanceRatio(config.size) : normalizeVideoSize(config.size),
        videoSeconds: normalizeVideoSeconds(config.videoSeconds, miniMaxH3),
        vquality: miniMaxH3 ? normalizeMiniMaxResolutionValue(config.vquality) : normalizeResolution(config.vquality),
        videoGenerateAudio: String(boolConfig(config.videoGenerateAudio, true)),
        videoWatermark: String(boolConfig(config.videoWatermark, false)),
    };
}

function normalizeVideoSeconds(value: string, unbounded = false) {
    if (String(value).trim() === "-1") return "-1";
    const seconds = Math.floor(Number(value) || 5);
    if (unbounded) return String(Math.max(1, seconds));
