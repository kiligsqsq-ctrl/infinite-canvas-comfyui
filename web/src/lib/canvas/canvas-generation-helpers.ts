    ...
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if ((node.type === CanvasNodeType.Video || node.type === CanvasNodeType.Audio) && node.metadata?.storageKey) {
                const resolved = await resolveMediaUrl(node.metadata.storageKey, "");
                return { ...node, metadata: { ...node.metadata, content: resolved, missingMedia: !resolved } };
            }
            if (node.type !== CanvasNodeType.Image) return node;
            const images = await Promise.all((node.metadata?.images || []).map(async (image) => {
                if (!image.storageKey) return image;
                const resolved = await resolveImageUrl(image.storageKey, "");
                return { ...image, content: resolved, missingMedia: !resolved };
            }));
            if (node.metadata?.storageKey) {
                const resolved = await resolveImageUrl(node.metadata.storageKey, "");
                return { ...node, metadata: { ...node.metadata, content: resolved, images, missingMedia: !resolved } };
            }
