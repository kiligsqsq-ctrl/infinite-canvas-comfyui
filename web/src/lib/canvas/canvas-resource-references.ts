...
export function buildReferenceConnectionLabels(targetNodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const labelsByNodeId = new Map(labelResourceNodes(getContextResourceNodes(targetNodeId, nodes, connections), true).map((reference) => [reference.nodeId, reference.label]));
    return new Map(connections.filter((connection) => connection.toNodeId === targetNodeId).flatMap((connection) => {
        const label = labelsByNodeId.get(connection.fromNodeId);
        return label ? [[connection.id, label] as const] : [];
    }));
}
