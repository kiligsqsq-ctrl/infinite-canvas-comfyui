...
import { buildApiUrl, resolveModelRequestConfig, resolveModelWorkflowId, type AiConfig, type ModelChannel } from "@/stores/use-config-store";
import { runComfyWorkflow } from "./comfy-workflow";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    const workflowId = resolveModelWorkflowId(config, config.model || config.imageModel);
    if (workflowId) {
        ...
        try {
            ...
            return [{ id: nanoid(), dataUrl: URL.createObjectURL(result) }];
        } catch (error) {
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">, options?: { timeout?: number }) {
    try {
        if (config.apiFormat === "gemini") {
            const response = await axios.get<GeminiPayload>(geminiApiUrl({ ...defaultGeminiConfig, ...config }), { headers: geminiHeaders({ ...defaultGeminiConfig, ...config }), timeout: options?.timeout });
...
export async function fetchChannelModels(channel: ModelChannel, options?: { timeout?: number }) {
