...
import { buildApiUrl, channelRequiresApiKey, resolveModelRequestConfig, resolveModelWorkflowId, type AiConfig } from "@/stores/use-config-store";
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob> {
    ...
    const workflowId = resolveModelWorkflowId(config, config.model || config.audioModel);
    if (workflowId) {
        if (!model) throw new Error(apiText("audioModelRequired"));
        if (!requestConfig.baseUrl.trim()) throw new Error(apiText("baseUrlRequired"));
        if (!requestConfig.apiKey.trim() && channelRequiresApiKey(requestConfig)) throw new Error(apiText("apiKeyRequired"));
        try {
            return await runComfyWorkflow({
                capability: "audio",
                workflowId,
