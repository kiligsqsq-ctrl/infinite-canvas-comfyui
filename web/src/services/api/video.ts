...
import { buildApiUrl, channelRequiresApiKey, modelOptionName, resolveModelRequestConfig, resolveModelWorkflowId, type AiConfig } from "@/stores/use-config-store";
import { runComfyWorkflow, type ComfyMediaReference } from "./comfy-workflow";
...
export type VideoGenerationTask = { id: string; provider: "openai" | "seedance" | "comfyui"; model: string };
export type VideoGenerationTaskState = { status: "pending" } | { status: "completed"; result: VideoGenerationResult } | { status: "failed"; error: string };

/** ComfyUI workflow results run to completion while the task is created, then use the shared task interface. */
const comfyVideoResults = new Map<string, VideoGenerationResult>();
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    const workflowId = resolveModelWorkflowId(config, selectedModel);
    if (workflowId) return createComfyVideoTask(requestConfig, selectedModel, workflowId, prompt, references, videoReferences, audioReferences, options);
    if (requestConfig.serviceType === "comfyui") throw new Error("请先在配置中为这个模型上传并保存 ComfyUI API 工作流");
...
export async function pollVideoGenerationTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    if (task.provider === "comfyui") {
        const result = comfyVideoResults.get(task.id);
        if (!result) return { status: "failed", error: apiText("videoGenerationFailed") };
        comfyVideoResults.delete(task.id);
...
async function createComfyVideoTask(config: AiConfig, model: string, workflowId: string, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], options?: RequestOptions): Promise<VideoGenerationTask> {
    if (!config.baseUrl.trim()) throw new Error(apiText("baseUrlRequired"));
    if (!config.apiKey.trim() && channelRequiresApiKey(config)) throw new Error(apiText("apiKeyRequired"));
    const [refs, videos, audios] = await Promise.all([
        Promise.all(references.map((image) => imageToDataUrl(image))),
        Promise.all(videoReferences.map(toComfyMediaReference)),
        Promise.all(audioReferences.map(toComfyMediaReference)),
    ]);
    const result = {
        blob: await runComfyWorkflow({
            capability: "video",
            workflowId,
            config,
            prompt,
            ...
            audios,
            params: {
                seconds: normalizeVideoSeconds(config.videoSeconds, model),
                size: normalizeVideoSize(config.size),
                resolution: normalizeVideoResolution(config.vquality, model),
                ratio: config.size,
    ...
    const id = nanoid();
    comfyVideoResults.set(id, result);
    return { id, provider: "comfyui", model };
}

async function toComfyMediaReference(reference: ReferenceVideo | ReferenceAudio): Promise<ComfyMediaReference> {
    let blob = reference.storageKey ? await getMediaBlob(reference.storageKey) : null;
    if (!blob && /^(blob:|data:)/i.test(reference.url)) {
        try {
            const response = await fetch(reference.url);
            if (response.ok) blob = await response.blob();
        } catch {
            // Keep the original URL available when a browser-local URL can no longer be resolved.
        }
    }
    return {
        name: reference.name,
...
function normalizeVideoSeconds(value: string, model = "") {
    ...
    if (/minimax[\s_-]*h3/i.test(model)) return String(Math.max(1, seconds));
...
function normalizeVideoSize(value: string) {
    ...
    const pixels = 900_000;
    const width = Math.ceil(Math.sqrt(pixels * ratio) / 32) * 32;
    const height = Math.ceil(Math.sqrt(pixels / ratio) / 32) * 32;
    return `${width}x${height}`;
}

function normalizeVideoResolution(value: string, model = "") {
    if (/minimax[\s_-]*h3/i.test(model)) return value === "0.9" ? "0.9" : "0.4";
    if (value === "low") return "480p";
    if (value === "auto" || value === "high" || value === "medium") return "720p";
    if (value === "0.4" || value === "0.9") return "720p";
